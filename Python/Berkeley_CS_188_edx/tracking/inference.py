...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            # if the ghost has been captured, update the beliefs
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        #all possible locations within the beliefs
        for oldPos in self.beliefs.keys():
            # get the P(new Position|old position) aka P(X_t|X_t-1)
            newPosAndDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos in newPosAndDist.keys():
                newPosDist[newPos] += newPosAndDist[newPos]*self.getBeliefDistribution()[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles=[]
        self.beliefs = util.Counter()
        positionSize = len(self.legalPositions)
        for i in range(0,self.numParticles-1):
            #use modulation to assign self.particles[i] with corresponding position, resulting evently distribution
            positionIndex = i%positionSize
            self.particles.append(self.legalPositions[positionIndex])
# the above can be also written as
        # self.beliefs = util.Counter()
        # totalPosSize = len(self.legalPositions)
        # self.particles = [self.legalPositions[p%totalPosSize] for p in range(1, self.numParticles)]
    ...
    def observe(self, observation, gameState):
        ...
        if(noisyDistance == None):
            ...
            keysToPacmanDistance = util.Counter()
            newBeliefs = util.Counter()
            for p in self.beliefs.keys():
                keysToPacmanDistance[p] = util.manhattanDistance(p, pacmanPosition)
                newBeliefs [p] = emissionModel[keysToPacmanDistance[p]] * self.beliefs[p]
            if (newBeliefs.totalCount() == 0):
                self.initializeUniformly(gameState)
            else:
                newBeliefs.normalize()
                self.particles = [util.sample(newBeliefs) for i in range(1,self.numParticles)]
            self.getBeliefDistribution()
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        newPosDist = util.Counter()
        storeNewDis = util.Counter()
        for p in self.beliefs.keys():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState,p))
            newPosDist.divideAll(1/self.getBeliefDistribution[p])# all the transition state(P(x_t|x_t-1)) multiple by the previous state given the previous evident(P(x_t-1|y_t-1))
            storeNewDis += newPosDist
        storeNewDis.normalize()
        self.particles = [util.sample(storeNewDis) for i in range(1,self.numParticles)]
        self.getBeliefDistribution()
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        positionsSum = util.Counter()
        for i in range(0, len(self.particles)):
            location = self.particles[i]
            positionsSum[location] += 1
        positionsSum.normalize()
        self.beliefs = positionsSum
        return self.beliefs
# above can be written as
        # counters = util.Counter()
        # reduce(lambda x,y: counters.incrementAll([y], 1), self.particles, counters)
        # counters.normalize()
        # self.beliefs = counters
        # return self.beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        totalPosSize = len(self.legalPositions)
        ghostPositionUniformList = [self.legalPositions[p%totalPosSize] for p in range(0, self.numParticles)]
        multipleGhostsPositionUniformList = []
        for i in range(0, self.numGhosts):
            random.shuffle(ghostPositionUniformList)#shuffle the sequence of uniform distributed list of tuples
            multipleGhostsPositionUniformList.append(tuple(ghostPositionUniformList))
        self.particles = zip(* multipleGhostsPositionUniformList)
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        def updateCapturedGhostPos():
            tmpStore = [self.getParticleWithGhostInJail(particle, i) for particle in self.particles]
            self.particles = tmpStore
            self.getBeliefDistribution()

        store = util.Counter()
        for i in range(0, self.numGhosts):
            if(noisyDistances[i] == None):
                updateCapturedGhostPos()
            else:
                for ghostsPos in self.beliefs.keys():
                    ghostsDistance = [util.manhattanDistance(p, pacmanPosition) for p in ghostsPos]
                    store[ghostsPos] = emissionModels[i][ghostsDistance[i]]*self.getBeliefDistribution()[ghostsPos]
                store.normalize() #nomalize all the probability at all locations for each ghost
                if store.totalCount() == 0: #check whether sum of all beliefs become 0
                    self.initializeParticles()
                    if(noisyDistances[i] == None):
                        updateCapturedGhostPos()
                    break #break if all positions have probability of 0, jump out from the most upper loop(range(0,self.numGhosts))
                else: #generate the updated praticles for each ghost
                    self.particles = [util.sample(store) for k in range(0,self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            ghostSamples = []
            for i in range(self.numGhosts):
                nextGhostPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i, self.ghostAgents[i])
                prevPosUpdate = self.getBeliefDistribution()[oldParticle]
                nextGhostPosDist.divideAll(1/prevPosUpdate)
                singleGhostSample = util.sample(nextGhostPosDist)
                ghostSamples.append(singleGhostSample)
            newParticle = ghostSamples
            "*** END YOUR CODE HERE ***"
