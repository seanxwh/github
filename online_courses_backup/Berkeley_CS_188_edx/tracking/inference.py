...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if(noisyDistance == None):
            ...
            legalPositionsToPacDistance = util.Counter()
            for pos in self.legalPositions:  # find Manhattan distance for any legalPosition to pacman
                legalPositionsToPacDistance[pos] = util.manhattanDistance(pos, pacmanPosition)
            positionsCounter = util.Counter()
            for position in self.particles:
                positionsCounter[position] += emissionModel[legalPositionsToPacDistance[position]]
            if positionsCounter.totalCount()==0:
                self.initializeUniformly(gameState)
            else:
                positionsCounter.normalize()
                self.particles = [util.sample(positionsCounter) for i in range(0,self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ghostDist = self.getPositionDistribution(self.setGhostPosition(gameState,particle))
            nextPos = util.sample(ghostDist)
            particlesStorage.append(nextPos)
        self.particles = particlesStorage
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        def updateCapturedGhostPos(ghostIndex):
            tmpStore = [self.getParticleWithGhostInJail(particle, ghostIndex) for particle in self.particles]
        ...
        #     if(noisyDistances[i] == None):  # the case of ghost has been captured
        #         updateCapturedGhostPos(i)
        #     else:
        #         ghostEmissionsList = []
        #         for particle in self.particles:
        #             ghostLocation = particle[i]
        #             ghostManhattanDistance = legalPositionsToPacDistance[ghostLocation]
        #             ghostEmissionModel = emissionModels[i][ghostManhattanDistance]
        #             ghostEmissionsList.append(ghostEmissionModel)
        #         ghostsEmissionsList.append(ghostEmissionsList)
        # newGhostsEmissionsList = reduce(lambda x,y: [a*b for a, b in zip(x, y)], ghostsEmissionsList, [1]*self.numParticles)
        # newPosDistCounter = util.Counter()
        # for i in range (0, self.numParticles):
        #     position = self.particles[i]
        #     newPosDistCounter[position] += newGhostsEmissionsList[i]
        # if  newPosDistCounter.totalCount()==0:
        #     self.initializeParticles()
        #     for i in range(0, self.numGhosts):
        #         if(noisyDistances[i] == None):  # the case of ghost has been captured
        #             updateCapturedGhostPos(i)
        # else:
        #     newPosDistCounter.normalize()
        #     self.particles = [util.sample(newPosDistCounter) for i in range(0,self.numParticles)]
        # self.getBeliefDistribution()
        #
        positionsCounter = util.Counter()
        for particle in self.particles:
            ghostEmissionsCounter = []
            particleCounter = 1
            for i in range(0, self.numGhosts):
                if(noisyDistances[i] == None):  # the case of ghost has been captured
                    ...
                    ghostManhattanDistance = legalPositionsToPacDistance[ghostLocation]
                    ghostEmissionModel = emissionModels[i][ghostManhattanDistance]
                    particleCounter *= ghostEmissionModel
            positionsCounter[particle] += particleCounter
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newGhostPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),
                                                   i, self.ghostAgents[i])
                newParticle[i] = util.sample(newGhostPosDist)
            "*** END YOUR CODE HERE ***"
