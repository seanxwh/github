...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    additionalFeatures = util.Counter()

    for key in features.keys():
        xCoor,yCoor = key

        if (xCoor,yCoor+1) in features.keys() and (xCoor,yCoor+2) in features.keys():
            additionalFeatures[((xCoor,yCoor+1),(xCoor,yCoor+2),key)] = (features[(xCoor,yCoor+1)]+features[(xCoor,yCoor+2)]+features[key])**2 #sum of the curr pix and next two pix up
            additionalFeatures[((xCoor,yCoor+1),key)] = (features[key]-features[(xCoor,yCoor+1)])*features[key] #difference between current pix and nx pix up
            additionalFeatures[((xCoor,yCoor+2),key)] = features[key]-features[(xCoor,yCoor+2)] #difference between current pix and nx nx pix up

        elif (xCoor,yCoor-1) in features.keys() and (xCoor,yCoor-2) in features.keys():
            additionalFeatures[((xCoor,yCoor-1),(xCoor,yCoor-2),key)] = (features[(xCoor,yCoor-1)]+features[(xCoor,yCoor-2)]+features[key])**2
            additionalFeatures[((xCoor,yCoor-1),key)] = (features[key]-features[(xCoor,yCoor-1)])*features[key]
            additionalFeatures[((xCoor,yCoor-2),key)] = features[key]-features[(xCoor,yCoor-2)]

        elif (xCoor+1,yCoor) in features.keys() and (xCoor+2,yCoor) in features.keys():
            additionalFeatures[((xCoor+1,yCoor),(xCoor+2,yCoor),key)] = (features[(xCoor+1,yCoor)]+features[(xCoor+2,yCoor)]+features[key])**2
            additionalFeatures[((xCoor+1,yCoor),key)] = (features[key]-features[(xCoor+1,yCoor)])*features[key]
            additionalFeatures[((xCoor+2,yCoor),key)] = features[key]-features[(xCoor+2,yCoor)]
    ...
    features.__radd__(additionalFeatures)
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    #refer to Prj 2 mutiAgent and its heuristicFunction
    currentGameState = state
    successorGameState = currentGameState.generateSuccessor(0, action)
    foodCount = successorGameState.getFood().count()
    ...
    if FoodsDst:
        ...
        ghostsDstScore = sum(ghostsDst)
        features['Total_Ghost_Dst'] = ghostsDstScore
        features['Min_Ghost_Dst'] = min(ghostsDst)
        features['Food_Count'] = foodCount
        features['Min_Food_Dist'] = -minFoodsDst
    features['Game_Score'] = successorGameState.getScore()
    return features
